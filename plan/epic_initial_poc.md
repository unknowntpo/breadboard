...
- [x] **Adopt clean architecture** - Domain, Repository, Service, API layers
  ...
  - [x] **Airflow K8s Deployment** (Phase 1.6 - Helm)
    - [x] Modified `backend/infrastructure/airflow_tasks.py` (added service initialization for Airflow context)
    - [x] `airflow/Dockerfile` (Custom image with DAGs baked in)
    - [x] `k8s/airflow-values.yaml` (Helm values override)
    - [x] Modified `Tiltfile` (added Airflow docker_build + helm + k8s_resource)
    - [x] Updated `.gitignore` (Helm artifacts + Airflow)
